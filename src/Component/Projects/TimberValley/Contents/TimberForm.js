...
const TimberForm = () => {
  ...
  const submitToWeb3Forms = async (formData) => {
    try {
      const response = await fetch('https://api.web3forms.com/submit', {
        ...
        body: JSON.stringify({
          access_key: 'c225c50d-a419-4ade-ad75-e53473c5e568', // Replace with your actual Web3Forms access key
          name: formData.name,
          email: formData.email,
          whatsapp: formData.whatsapp,
          subject: 'New Lead from Timber Valley Form',
      ...
      console.log('Web3Forms API Response:', responseData); // Logging response for verification
    } catch (error) {
      console.error('Error submitting to Web3Forms:', error);
      throw error; // Propagate error for better error handling in handleSubmit
    }
  };
  ...
  return (
    <div className="form-container position-relative">
      ...
      >
        <h3 className="text-center para-color3 mb-4 fs-4">
          The best time to invest is now. Act fast!{" "}
        </h3>
        ...
        <div className="form-group">
          ...
          />
          {errors.whatsapp && (
            <p style={{ color: "red", fontSize: "12px" }}>{errors.whatsapp}</p>
          )}
        </div>
        <div className="form-group">
